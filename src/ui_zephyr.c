...
ui_task_handle_t ui_handle;

static struct k_event ui_events;
...
static void ui_tick_timer_cb(struct k_timer *timer)
{
    (void)timer;
    k_event_post(&ui_events, UI_EVT_TICK);
}

K_TIMER_DEFINE(ui_tick_timer, ui_tick_timer_cb, NULL);
...
void ui_led_set_pattern(uint8_t led_idx, uint32_t pattern, bool loop)
{
    ...
    if (ui_handle)
        k_event_post(&ui_events, led_idx ? UI_EVT_LED1 : UI_EVT_LED0);
}
...
void ui_ws2812_set(uint32_t color, uint32_t pattern, bool loop)
{
    ...
    ui_ws2812_loop = loop;
    if (ui_handle)
        k_event_post(&ui_events, UI_EVT_WS2812);
}
...
void ui_notify_output_event(const char *s)
{
    ...
    k_event_post(&ui_events, UI_EVT_OUTPUT_EVENTS);
}
...
void ui_notify_layout(const char *name)
{
    ...
    if (ui_handle)
        k_event_post(&ui_events, UI_EVT_LAYOUT);
}
...
static void ui_render_status_screen(ui_state_t *ui)
{
    ...
    ssd1306_clear(ui->disp);
    ssd1306_putn6x8(ui->disp, 0, UI_STATUS_MODE_COL, "BLE", UI_STATUS_MODE_LEN);
    ui->cdc_connected = ui_cdc_connected;
...
void ui_task(void *pvParameters)
{
    ...
    k_event_init(&ui_events);
    ui_handle = k_current_get();

    ui_state_t ui = {0};

    for (uint8_t i = 0; i < config->nr_leds; ++i)
        platform_gpio_output(config->led_pins[i], false);

    bool has_ws2812 = config->ws2812_pin != -1 && ws2812_hw_init(config->ws2812_pin);

    bool has_display = config->ssd1306.i2c_idx != -1;
    if (has_display) {
        for (uint8_t bus = 0; bus < MAX_I2C; ++bus) {
            if (!(config->i2c_mask & (uint8_t)(1u << bus)))
                continue;
            platform_i2c_init(bus,
                              config->i2c[bus].sda,
                              config->i2c[bus].scl,
                              config->i2c[bus].baud);
        }
    ...
    k_timer_start(&ui_tick_timer, K_MSEC(UI_STEP_MS), K_MSEC(UI_STEP_MS));

    for (;;) {
        uint32_t events = k_event_wait(&ui_events,
                                       UI_EVT_LED0 | UI_EVT_LED1 | UI_EVT_WS2812 |
                                       UI_EVT_SCREEN | UI_EVT_STATUS | UI_EVT_TICK |
                                       UI_EVT_OUTPUT_EVENTS | UI_EVT_LAYOUT,
                                       true,
                                       K_FOREVER);
        ...
        if (has_display) {
            ...
            if ((events & UI_EVT_STATUS) && ui.screen == UI_SCREEN_STATUS)
                ui_update_status_indicators(&ui);
        }
        ...
        for (uint8_t i = 0; i < config->nr_leds; ++i) {
            platform_gpio_write(config->led_pins[i], (led_ring[i] & 0x01u) != 0);
        ...
        if (has_display && ui.screen == UI_SCREEN_STATUS)
