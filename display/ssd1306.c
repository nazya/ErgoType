...
static void ssd1306_i2c_write(const ssd1306_cfg_t *cfg, const uint8_t *data, size_t n)
{
#ifdef ERGOTYPE_ZEPHYR
    (void)platform_i2c_write((uint8_t)cfg->i2c_idx, cfg->addr, data, n);
#else
    i2c_inst_t *i2c = ssd1306_i2c(cfg);
    (void)i2c_write_blocking(i2c, cfg->addr, data, n, false);
#endif
}
...
static void ssd1306_write_cmds(const ssd1306_cfg_t *cfg, const uint8_t *cmds, size_t n)
{
    ...
    while (n) {
        ...
        ssd1306_i2c_write(cfg, buf, chunk + 1);
        cmds += chunk;
        n -= chunk;
    }
}
