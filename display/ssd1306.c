...
static inline platform_i2c_t *ssd1306_i2c(const ssd1306_cfg_t *cfg)
{
...
static void ssd1306_write_cmds(const ssd1306_cfg_t *cfg, const uint8_t *cmds, size_t n)
{
    ...
    platform_i2c_t *i2c = ssd1306_i2c(cfg);
    ...
    while (n) {
        ...
        (void)platform_i2c_write_blocking(i2c, cfg->addr, buf, chunk + 1, false);
        cmds += chunk;
        n -= chunk;
    }
}
