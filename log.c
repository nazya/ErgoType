...
static const char *colorize(const char *s)
{
	...
	static char buf[PLATFORM_CDC_BUFFER_SIZE];
...
typedef struct {
	char buf[PLATFORM_CDC_BUFFER_SIZE];
	size_t len;
} log_sink_t;
...
{
	if (sink->len)
		platform_cdc_write(sink->buf, sink->len);
	sink->len = 0;
}
...
static void vmsg(int level, const char *fmt, va_list ap)
{
	...
	log_sink_t sink = {0};
	platform_vfctprintf(sink_out, &sink, colorize(fmt), ap);
...
// 	platform_cdc_write(prefix, strlen(prefix));
