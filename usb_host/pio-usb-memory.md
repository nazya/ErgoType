...
unchanged `HID_MAX_FIELDS/USAGES=64/675`. The native HID-BPF adapter and all
retained program sources are commented out; `hid-rakk.c` also remains
commented out. Corsair K90, original Roccat Kone, and Prodikeys
keyboard input are active. The test observer is absent. The final comment-only
CMake selection has not been rebuilt, so current source measurements are
pending:

```text
firmware build ID            pending rebuild
text/data/bss                pending rebuild
__bss_end__                  pending rebuild
main-bank headroom           pending rebuild
ELF/UF2 SHA-256              pending rebuild
hardware verdict             not tested after source selection cleanup
```

The active upstream `!CONFIG_HID_BPF` stubs allocate no adapter, scratch, or
private program state. When the retained adapter declarations/source and a
program source are restored, each matching HID separately owns its
adapter/private state; unmatched devices allocate none. Event programs allocate scratch at connect,
sized to their largest parsed report rounded up to 64 bytes, and release it at
disconnect. Private state lasts until attachment destruction. Huion firmware
properties additionally use 64 heap bytes. These costs are in addition to
ordinary HID parser, report, input, and transport allocations.

Probe arguments temporarily include a 4096-byte descriptor plus metadata.
They overlap the candidate adapter/private allocation while `probe()` runs,
then are freed before mode requests; a rejected candidate allocation is also
freed. Huion string exchange uses a separate transient 256-byte buffer.
Descriptor replacement uses a 4096-byte workspace plus the final descriptor
copy briefly at the same time; the workspace is then freed. ACK05 output
copies its 32-byte request for the existing transport and uses one reusable
delayed-work item, not a new task. These are source-derived allocation costs,
excluding allocator overhead and other concurrent owners, not measured runtime
peaks.
...
then-default-off production image rebuilt. Compilation and size alone are not
runtime evidence; exact log hashes and scope limits are in
