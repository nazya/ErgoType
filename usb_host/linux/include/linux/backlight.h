...
// Upstream backlight is device-core/sysfs infrastructure. Firmware keeps
// the class device contract for HID drivers and future proxy/control state.
...
#define BACKLIGHT_PROXY_ENUM_BEGIN ((__u32)-1)
#define BACKLIGHT_PROXY_STRING 64
...
struct backlight_proxy_snapshot {
	__u32 proxy_id;
	struct backlight_properties props;
	__s32 use_count;
	__u8 has_ops;
	__u8 has_update_status;
	__u8 has_get_brightness;
	__u8 has_controls_device;
	char name[BACKLIGHT_PROXY_STRING];
};
...
int backlight_device_for_each(int (*fn)(struct backlight_device *bd, void *data), void *data);
int backlight_proxy_get_snapshot(__u32 proxy_id,
				 struct backlight_proxy_snapshot *snapshot);
int backlight_proxy_get_next_snapshot(__u32 after_proxy_id,
				      struct backlight_proxy_snapshot *snapshot);
int backlight_proxy_set_brightness(__u32 proxy_id, unsigned long brightness);
