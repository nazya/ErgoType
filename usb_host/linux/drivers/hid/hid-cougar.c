...
// Upstream's DEFINE_MUTEX() constructs a ready static mutex. The FreeRTOS
// compatibility object owns a heap-backed semaphore, so initialize it before
// any HID device can bind.
static int __init cougar_init(void)
{
	mutex_init(&cougar_udev_list_lock);
	return mutex_initialized(&cougar_udev_list_lock) ? 0 : -ENOMEM;
}
module_init(cougar_init);
				   ...
				   struct cougar *cougar)
{
	...
	// 	return error;
	// }
	// Downstream fix: Linux and firmware devm_add_action_or_reset() invoke the
	// action when registration fails. Register after unlocking so the final
	// kref callback cannot recursively acquire cougar_udev_list_lock.

out:
	mutex_unlock(&cougar_udev_list_lock);
	// return error;
	// Preserve allocation errors; a successful bind instead returns cleanup
	// registration after the firmware mutex has been released.
	if (error)
		return error;
			...
			const struct hid_device_id *id)
{
	...
	// Linux's static DEFINE_MUTEX() cannot fail. Firmware records initcall
	// failure and continues registering unrelated drivers, so do not use a
	// missing FreeRTOS semaphore if this driver's initialization failed.
	if (!mutex_initialized(&cougar_udev_list_lock))
		return -ENOMEM;
...
static const struct hid_driver cougar_driver = {
	.name			= "cougar",
