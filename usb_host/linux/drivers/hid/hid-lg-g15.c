...
#include <linux/hid.h>
#if 0 /* Input-only port excludes LED/backlight control. */
...
static int lg_g15_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	case LG_G13:
		...
		break;
	case LG_G15:
#if 0 /* Input-only port has no LED worker. */
...
#if 0 /* Z-10 speakers are outside the selected gaming-input scope. */
	/* Z-10 speakers */
	{ HID_USB_DEVICE(USB_VENDOR_ID_LOGITECH,
...
static const struct hid_driver lg_g15_driver = {
	.name			= "lg-g15",
