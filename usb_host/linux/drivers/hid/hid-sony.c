...
#if 0
/* Controller-only LED, battery, Bluetooth, USB URB and multitouch dependencies. */
...
#if 0
/* Only USB VAIO RF receivers are linked; retain controller descriptors and state. */
...
struct sony_sc {
#if 0
	/* Controller synchronization and connection tracking are not used by VAIO. */
	spinlock_t lock;
	struct list_head list_node;
#endif
	struct hid_device *hdev;
#if 0
	/* Controller input, touchpad, sensor and LED state. */
...
#if 0
	/* The selected VAIO IDs have no controller output, battery or timer state. */
...
#if 0
/* Controller mapping and output paths are outside the USB VAIO receiver port. */
...
#if 0
	/* Controller quirks are excluded with their device IDs. */
...
#if 0
/* Controller descriptor fixups remain disabled with their device IDs. */
...
#if 0
/* Controller reports, LEDs, battery, Bluetooth identity and input setup are unlinked. */
...
static int sony_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	// VAIO receivers do not allocate controller URBs.
	unsigned int connect_mask = HID_CONNECT_DEFAULT;

#if 0
/* Controller name overrides do not apply to the selected VAIO receiver IDs. */
	...
	// The VAIO fixup has no shared controller state to synchronize.
...
#if 0
/* Controller HIDDEV/version behavior is disabled with the controller IDs. */
...
#if 0
/* Instrument initialization and USB URBs are not part of VAIO receiver support. */
...
err:
	...
	// VAIO receivers have no controller URB allocation to unwind.
...
{
#if 0
/* Controller periodic URBs are never created for the selected VAIO receivers. */
	struct sony_sc *sc = hid_get_drvdata(hdev);
	...
	// hid_hw_close(hdev);
	// Only controller battery setup takes a driver-owned open reference;
	// hid_hw_stop() disconnects input and closes its own VAIO open reference.

#if 0
	/* No controller workers, Bluetooth list entries or device IDs exist for VAIO. */
	sony_cancel_work_sync(sc);
...
#if 0
/* Controller power-management hooks are not used by USB VAIO receivers. */
...
#if 0
/* Only VAIO RF receiver IDs are enabled; controllers and Bluetooth stay excluded. */
	...
	{ HID_USB_DEVICE(USB_VENDOR_ID_SONY, USB_DEVICE_ID_SONY_VAIO_VGP_MOUSE),
		.driver_data = VAIO_RDESC_CONSTANT },
#if 0
/* Remaining controller, instrument and Bluetooth IDs are intentionally unlinked. */
	/*
...
// Firmware keeps driver descriptors in flash and registration state separately.
static const struct hid_driver sony_driver = {
	.name             = "sony",
	.id_table         = sony_devices,
#if 0
/* VAIO uses generic HID input mapping; these callbacks only set up controller state. */
...
#if 0
/* Controller raw reports and power-management callbacks remain unlinked. */
...
#if 0
/* Firmware uses its linker registry instead of module load/unload and controller IDA state. */
...
// Upstream init/exit above register the same driver; firmware collects this entry.
