...
// The port's USB HID transport is separate; this driver uses no usbhid internals.
#include "hid-ids.h"
#include "hid-lg.h"
#include "hid-lg4ff.h"
...
#if 0 /* Wheel descriptors are outside the enabled keyboard/mouse IDs. */
...
#if 0 /* SpaceNavigator and wheel IDs are not enabled in this port. */
...
#if 0 /* Wheel IDs are not enabled in this keyboard/mouse slice. */
...
#if 0 /* Wheel force feedback is not linked. */
...
{
#if 0 /* Only the disabled G29 path uses USB interface selection. */
...
#if 0 /* G29 wheel interface selection is outside the enabled IDs. */
...
#if 0 /* Force feedback is outside this keyboard/mouse slice. */
...
#if 0 /* Wii setup and wheel force feedback are not linked. */
...
#if 0 /* Only the disabled post-start wheel setup reaches this label. */
err_stop:
	hid_hw_stop(hdev);
#endif
err_free:
...
#if 0 /* Joystick, wheel/FF and SpaceNavigator devices remain deferred. */
...
static const struct hid_driver lg_driver = {
	.name = "logitech",
	.id_table = lg_devices,
