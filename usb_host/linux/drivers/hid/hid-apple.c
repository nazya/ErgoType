...
// The selected USB table excludes legacy, internal, and Touch Bar backlights.
// Keep the pinned backlight implementation visible behind the broader gate.
#if IS_ENABLED(CONFIG_HID_APPLE_ALL_DEVICES)
...
#if IS_ENABLED(CONFIG_HID_APPLE_ALL_DEVICES) /* Gated backlight state. */
...
static int apple_fetch_battery(struct hid_device *hdev)
{
	...
	// Firmware omits the sparse 256-pointer hash to retain RP2040 host RAM.
...
#if IS_ENABLED(CONFIG_HID_APPLE_ALL_DEVICES) /* Gated backlight implementation. */
...
#if IS_ENABLED(CONFIG_HID_APPLE_ALL_DEVICES) /* Gated backlight probe path. */
...
#if IS_ENABLED(CONFIG_HID_APPLE_ALL_DEVICES) /* Only gated backlight init reaches this unwind. */
out_err:
	if (quirks & APPLE_RDESC_BATTERY)
...
static const struct hid_device_id apple_devices[] = {
	// Keep every pinned row in order; only the covered external USB set is
	// active.
...
static const struct hid_driver apple_driver = {
	.name = "apple",
