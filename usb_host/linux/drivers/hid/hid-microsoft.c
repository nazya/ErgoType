...
struct ms_data {
	...
	unsigned int last_key; /* Upstream static crosses HID device ownership. */
#if IS_ENABLED(CONFIG_HID_MICROSOFT_ALL_DEVICES)
	/*
	 * Bluetooth force-feedback devices are outside the firmware's wired USB
	 * scope. Keep their upstream state visible for a later complete port.
	 */
...
#if IS_ENABLED(CONFIG_HID_MICROSOFT_ALL_DEVICES)
/* Surface Dial is Bluetooth-only and outside the selected USB table. */
		...
		struct hid_usage *usage, __s32 value)
{
	...
	if (quirks & MS_ERGONOMY && usage->hid == (HID_UP_MSVENDOR | 0xff05)) {
		...
		// The upstream static crosses hid_device ownership.
		unsigned int key = 0;
		switch (value) {
		...
		if (key) {
			...
			input_event(input, usage->type, ms->last_key, 0);
...
#if IS_ENABLED(CONFIG_HID_MICROSOFT_ALL_DEVICES)
	/*
	 * The pinned upstream call serves only the gated Bluetooth Xbox/8BitDo
	 * entries. The active wired USB table has no MS_QUIRK_FF caller.
	 */
	ret = ms_init_ff(hdev);
	if (ret)
...
static void ms_remove(struct hid_device *hdev)
{
	hid_hw_stop(hdev);
#if IS_ENABLED(CONFIG_HID_MICROSOFT_ALL_DEVICES)
	/* Paired with the gated upstream force-feedback initialization above. */
	ms_remove_ff(hdev);
#endif
}
...
#if IS_ENABLED(CONFIG_HID_MICROSOFT_ALL_DEVICES)
	/* SideWinder is outside the firmware's non-gaming USB input scope. */
...
#if IS_ENABLED(CONFIG_HID_MICROSOFT_ALL_DEVICES)
	/* Bluetooth and its Xbox/8BitDo force-feedback path are not linked. */
...
// Firmware builtin driver descriptors are immutable and live in flash.
static const struct hid_driver ms_driver = {
	.name = "microsoft",
	.id_table = ms_devices,
