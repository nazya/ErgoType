...
#if 0 /* Firmware has no Roccat character-device or sysfs configuration interface. */
#include <linux/hid-roccat.h>
#endif
// Upstream obtains USB declarations from hid-roccat-common.h.
#include <linux/usb.h>
#include "hid-ids.h"
#if 0 /* Firmware has no Roccat character-device or sysfs configuration interface. */
...
#if 0 /* Profile/DPI/settings transfers serve the unavailable Roccat/sysfs interface. */
...
static int kone_init_specials(struct hid_device *hdev)
{
...
#if 0 /* Firmware only retains the mouse-event state used by the input correction. */
	struct usb_device *usb_dev = interface_to_usbdev(intf);
#endif
	struct kone_device *kone;
#if 0 /* Roccat configuration and character-device setup are not linked. */
	int retval;
#endif
...
#if 0 /* No profile/DPI/settings consumer exists outside Roccat/sysfs. */
	...
	return 0;
#if 0 /* The remaining allocation has no later initialization failure. */
exit_free:
	kfree(kone);
	return retval;
#endif
...
{
	struct usb_interface *intf = to_usb_interface(hdev->dev.parent);
#if 0 /* No Roccat character-device registration accompanies the input state. */
	struct kone_device *kone;
#endif
...
#if 0 /* Profile/DPI publication only serves the unavailable Roccat/sysfs interface. */
...
#if 0 /* Firmware input processing has no Roccat/sysfs notification consumer. */
...
static const struct hid_driver kone_driver = {
		.name = "kone",
...
#if 0 /* Firmware uses its linker registry and has no Roccat/sysfs class. */
