...
// Port uses local compatibility include paths directly until linux/kernel.h exists.
#include <stdio.h>
#include <string.h>
				       ...
				       unsigned int application)
{
	...
	// RP2040 uses the existing sparse report list instead of an embedded hash.
...
static int open_collection(struct hid_parser *parser, unsigned type)
{
	...
	if (parser->collection_stack_ptr == parser->collection_stack_size) {
		...
		// 			    GFP_KERNEL);
		// FreeRTOS has no size-aware krealloc(); old size is known here.
		collection_stack = kmalloc(new_size * sizeof(unsigned int),
					   GFP_KERNEL);
		if (!collection_stack)
			return -ENOMEM;

		if (parser->collection_stack) {
			memcpy(collection_stack, parser->collection_stack,
			       parser->collection_stack_size * sizeof(unsigned int));
...
static int hid_parser_main(struct hid_parser *parser, struct hid_item *item)
{
	...
	// Local usage arrays are allocated on demand in this port; keep them for the next main item.
	parser->local.usage_index = 0;
	parser->local.usage_minimum = 0;
	parser->local.delimiter_depth = 0;
	parser->local.delimiter_branch = 0;
				       ...
				       unsigned int report_counts)
{
	...
	if (id == 0) {
		...
		// RP2040 keeps the full ID range in the existing sparse report list.
		report = NULL;
		list_for_each_entry(candidate,
				    &hid->report_enum[type].report_list, list)
			if (candidate->id == id) {
				report = candidate;
				break;
			}
		...
		const u8 *data)
{
	...
	// RP2040 keeps the full ID range in the existing sparse report list.
	report = NULL;
	list_for_each_entry(candidate, &report_enum->report_list, list)
		if (candidate->id == n) {
			report = candidate;
			break;
		}
...
static struct hid_driver_runtime *hid_driver_runtime(const struct hid_driver *hdrv)
{
	struct bus_type *bus = (struct bus_type *)&hid_bus_type;
	struct device_driver *drv;

	list_for_each_entry(drv, &bus->drivers, bus_node) {
		if (drv->hid_driver == hdrv)
			return hid_driver_runtime_from_driver(drv);
	}
		...
		size_t count)
{
	...
	// if (ret < 3)
	// 	return -EINVAL;
	// newlib sscanf() pulls a large scanf/float parser; parse the same hex words directly.
	bus = simple_strtoul(p, &endp, 16);
	if (endp == p)
		return -EINVAL;
	p = endp;
	vendor = simple_strtoul(p, &endp, 16);
	if (endp == p)
		return -EINVAL;
	p = endp;
	product = simple_strtoul(p, &endp, 16);
	if (endp == p)
		return -EINVAL;
	p = endp;
	driver_data = simple_strtoul(p, &endp, 16);
	if (endp == p)
		driver_data = 0;
...
struct hid_device *hid_allocate_device(void)
{
	...
	// hid_close_report(hdev);
	// No report state exists before descriptor parsing in this port.
		...
		const char *mod_name)
{
	...
	// The port stores mutable registration state outside immutable hid_driver descriptors.
	rt->driver.name = hdrv->name;
	rt->driver.bus = &hid_bus_type;
	rt->driver.owner = owner;
	rt->driver.mod_name = mod_name;
	rt->driver.hid_driver = hdrv;

	// INIT_LIST_HEAD(&hdrv->dyn_list);
	// spin_lock_init(&hdrv->dyn_lock);
	INIT_LIST_HEAD(&rt->dyn_list);
	spin_lock_init(&rt->dyn_lock);
...
// TinyUSB host task calls hid_core_init() before imported driver initcalls so hid_bus_type exists first.
...
// Upstream Linux uses module_init(hid_init); TinyUSB host task calls this explicitly.
