...
// Unused BPF data symbol; these callbacks have no mutable button state.
...
int probe(struct hid_bpf_probe_args *ctx)
{
	...
	/* Native attachment runs before transport start. The same initial
	 * output request runs in ack05_connect() once the transport is ready.
	 */
...
/* Firmware-only lifecycle boundary: upstream probe has a running transport,
 * while native descriptor attachment precedes hid_hw_start().
 */
static int ack05_connect(struct hid_bpf_ctx *hctx)
{
	if (hctx->hid->dev_rsize == VENDOR_DESCRIPTOR_LENGTH) {
		// The adapter owns initialized delayed work; preserve the USB error.
		return switch_to_raw_mode(hctx);
	}
...
// Native compilation has no BPF loader license section; SPDX above is retained.

/* Firmware-only registration at the existing hid-core HID-BPF hooks. */
HID_BPF_STATIC_PROGRAM(xppen_ack05_remote, probe, 0,
		       HID_BPF_STATIC_CONNECT_FALLBACK, ack05_connect);
