...
static const char EXPECTED_FIRMWARE_ID[] = "HUION_T216_";
...
struct huion_dial2_state {
	__u8 last_button_state;
	__u8 button;
};
#define last_button_state (state->last_button_state)
#define button (state->button)
...
int BPF_PROG(dial_2_fix_events, struct hid_bpf_ctx *hctx)
{
	...
	struct huion_dial2_state *state = hid_bpf_get_private(hctx);
	__u8 *data = hid_bpf_get_data(hctx, 0 /* offset */, 16 /* size */);
	// static __u8 button;
	// Kept in the per-HID state alongside last_button_state.
...
HID_BPF_STATIC_PROGRAM(inspiroy_dial2, probe, sizeof(struct huion_dial2_state),
