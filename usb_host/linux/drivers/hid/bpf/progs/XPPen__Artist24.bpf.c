...
HID_BPF_CONFIG(
	...
	// The firmware's existing UC-Logic driver already owns 0x092d.
);
...
// Linux gives each loaded BPF instance its own globals; native state is per HID.
struct xppen_artist24_state {
	__u8 prev_state;
};
...
int BPF_PROG(xppen_24_fix_eraser, struct hid_bpf_ctx *hctx)
{
	...
	struct xppen_artist24_state *state = hid_bpf_get_private(hctx);
	...
	prev_tip = !!(state->prev_state & TIP_SWITCH);
	...
	changed_state = state->prev_state ^ current_state;
...
HID_BPF_STATIC_PROGRAM(xppen_artist_24, probe, sizeof(struct xppen_artist24_state),
		       0, NULL);
