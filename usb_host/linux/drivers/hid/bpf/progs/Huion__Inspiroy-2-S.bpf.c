...
// Firmware reads the same property through the USB switcher, per HID.
#define UDEV_PROP_HUION_FIRMWARE_ID hid_bpf_get_firmware_id(hctx)
...
// The prefix is immutable and private to this native program.
static const char EXPECTED_FIRMWARE_ID[] = "HUION_T21j_";
...
/* The upstream loader gives each HID its own BPF globals. Native programs
 * keep the same state in the attachment instead of sharing it across HIDs. */
struct huion_inspiroy2_s_state {
	__u8 last_button_state;
	__u8 last_tip_state;
	__u8 last_sec_barrel_state;
	__u8 force_tip_down_count;
};
#define last_button_state (state->last_button_state)
#define last_tip_state (state->last_tip_state)
#define last_sec_barrel_state (state->last_sec_barrel_state)
#define force_tip_down_count (state->force_tip_down_count)
...
{
	/* Native attachment state has the lifetime of this hid_device. */
	struct huion_inspiroy2_s_state *state = hid_bpf_get_private(hctx);
...
// Native registration has no BPF ELF license section.
static const char _license[] __attribute__((unused)) = "GPL";

/* Native static registration replaces the userspace BPF attachment. */
HID_BPF_STATIC_PROGRAM(inspiroy_2, probe, sizeof(struct huion_inspiroy2_s_state),
		       HID_BPF_STATIC_HUION_SWITCHER, NULL);
