...
struct waltop_batteryless_state {
...
int BPF_PROG(waltop_fix_events, struct hid_bpf_ctx *hctx)
{
	...
	struct waltop_batteryless_state *state = hid_bpf_get_private(hctx);
	...
	// History belongs to this tablet, not the native program as a whole.
	__u8 tip_held = state->last_button_state & TIP_SWITCH;
	__u8 barrel_held = state->last_button_state & BARREL_SWITCH;
	...
	state->last_button_state = data[1];
...
HID_BPF_STATIC_PROGRAM(waltop_batteryless, probe,
		       sizeof(struct waltop_batteryless_state), 0, NULL);
