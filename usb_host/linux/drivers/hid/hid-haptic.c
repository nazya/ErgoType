...
/* Firmware policy: expose at most five application haptic effect slots. */
...
static void hid_haptic_destroy(struct ff_device *ff)
{
	...
	haptic->wq = NULL;
	/* FreeRTOS-backed mutexes own dynamically allocated semaphore handles. */
	mutex_destroy(&haptic->manual_trigger_mutex);
	mutex_destroy(&haptic->auto_trigger_mutex);
		    ...
		    struct hid_haptic_device **haptic_ptr)
{
	...
	// heap-backed semaphore and haptic probe must reject its allocation failure.
	if (!mutex_initialized(&haptic->auto_trigger_mutex)) {
		ret = -ENOMEM;
		goto duration_map;
	}
	...
	if (!mutex_initialized(&haptic->manual_trigger_mutex)) {
		ret = -ENOMEM;
		goto manual_mutex;
	}
	...
	// Firmware exposes five application slots; 96 Linux userspace slots exhaust
	// the shared heap before another interface of the composite device can probe.
	...
	if (ret) {
		// Linux removes this input device when haptic probe fails. The
		// multitouch port keeps the otherwise usable input device, so roll back
		// the capability published above before leaving dev->ff NULL.
		__clear_bit(FF_HAPTIC, dev->ffbit);
...
output_queue:
	destroy_workqueue(haptic->wq);
	haptic->wq = NULL;
/* The port's dynamically allocated mutexes need explicit error-path release. */
manual_mutex:
	mutex_destroy(&haptic->manual_trigger_mutex);
duration_map:
	mutex_destroy(&haptic->auto_trigger_mutex);
