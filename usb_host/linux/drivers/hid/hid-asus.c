...
// No ACPI symbols are used by this driver.
// #include <linux/cleanup.h>
// The firmware hid_compat.h already supplies the __free(kfree) contract.
// #include <linux/dmi.h>
// Retained internal-device paths need Linux DMI and are gated out below.
#include <linux/hid.h>
...
// Laptop backlight listeners require the unported ASUS WMI/spinlock boundary.
#if IS_ENABLED(CONFIG_HID_ASUS_ALL_DEVICES)
...
// Laptop fan-control WMI handling is outside the selected wired keyboard.
#if IS_ENABLED(CONFIG_HID_ASUS_ALL_DEVICES)
/*
...
// These configuration commands belong to the unselected laptop backlight.
#if IS_ENABLED(CONFIG_HID_ASUS_ALL_DEVICES)
...
// Keep the upstream WMI/backlight and Ally MCU implementation inactive until
// its platform services and locking are available; no successful stubs.
#if IS_ENABLED(CONFIG_HID_ASUS_ALL_DEVICES)
...
// Firmware has no PM callbacks; this resume body also requires backlight state.
#if IS_ENABLED(CONFIG_HID_ASUS_ALL_DEVICES)
...
static int asus_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	// Retained internal, I2C, Bluetooth, and T100 paths remain gated.
#if IS_ENABLED(CONFIG_HID_ASUS_ALL_DEVICES)
	/*
...
static const struct hid_device_id asus_devices[] = {
	// I2C, Bluetooth, T100, and platform-dependent backlight/Ally rows remain
	// in upstream order behind these gates.
#if IS_ENABLED(CONFIG_HID_ASUS_ALL_DEVICES)
...
static const struct hid_driver asus_driver = {
	.name			= "asus",
...
// Firmware has no module namespaces or linked ASUS WMI subsystem.
MODULE_LICENSE("GPL");
