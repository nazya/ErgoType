...
static const struct hid_device_id hid_quirks[] = {
	...
	/*
	 * Firmware boundary: keep cross-PID sibling profiles and the bootloader
	 * aligned with their compile-gated Wacom rows. The upstream quirk path lets
	 * unchanged hid-generic bind instead of letting the wildcard claim them.
	 */
...
static const struct hid_device_id hid_have_special_driver[] = {
	...
	// Match these reservations to the reduced ASUS table so excluded I2C,
	// Bluetooth, and platform-backlight rows continue to hid-generic.
...
// The firmware HID++ table excludes G920 force feedback; preserve its generic
// fallback while the corresponding upstream table entry remains disabled.
#if IS_ENABLED(CONFIG_HID_LOGITECH_HIDPP) && \
	IS_ENABLED(CONFIG_HID_LOGITECH_HIDPP_ALL_DEVICES)
...
#endif
// Upstream Linux keeps this row inside the complete CONFIG_HID_ROCCAT block.
// Firmware links only the original Kone correction at this stage.
#if IS_ENABLED(CONFIG_HID_ROCCAT_KONE)
