...
// The reduced firmware sysfs contract is supplied through linux/device.h.
...
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
...
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
	/* Tablet-only LED/work state stays with the gated upstream device rows. */
	u8 led_report[3]; /* Must be first for proper alignment */
#endif
#if IS_ENABLED(CONFIG_LEDS_CLASS)
	/* Linux LED-class state is absent from the selected firmware scope. */
	int led_state;
#endif
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
...
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
/* Tablet LED/work paths are retained for the gated upstream device rows. */
...
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
/* Dock, Bluetooth, and I2C report fixups are outside the selected USB rows. */
...
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
/* ScrollPoint and tablet mappings remain beside their gated upstream rows. */
...
static void lenovo_features_set_cptkbd(struct hid_device *hdev)
{
	...
	// Keep the Bluetooth half adjacent but outside the wired USB port scope.
	if (hdev->product == USB_DEVICE_ID_LENOVO_CUSBKBD
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
	    || hdev->product == USB_DEVICE_ID_LENOVO_CBTKBD
#endif
...
static int lenovo_features_set_tpkbd(struct hid_device *hdev)
{
	...
	// Firmware keeps the complete report-ID range in a sparse registry.
	report = hid_report_enum_lookup(&hdev->report_enum[HID_FEATURE_REPORT], 4);
...
#if IS_ENABLED(CONFIG_LEDS_CLASS)
/* Linux audio LED triggers have no producer in the selected firmware scope. */
static void lenovo_led_set_tpkbd(struct hid_device *hdev)
{
	...
	// Firmware keeps the complete report-ID range in a sparse registry.
	report = hid_report_enum_lookup(&hdev->report_enum[HID_OUTPUT_REPORT], 3);
...
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
/* Tablet LED/work probes remain beside their gated upstream device rows. */
...
#if IS_ENABLED(CONFIG_HID_LENOVO_ALL_DEVICES)
	/*
	 * Keep 60ee inactive in the RP2040 Lenovo driver: its full report graph
	 * exceeds the heap at 675 usages.
	 */
	{ HID_USB_DEVICE(USB_VENDOR_ID_LENOVO, USB_DEVICE_ID_LENOVO_TPIIUSBKBD) },
	/* Bluetooth, dock, ScrollPoint, tablet, and I2C rows are not selected. */
...
// Firmware builtin-driver registration keeps pinned driver metadata immutable.
static const struct hid_driver lenovo_driver = {
	.name = "lenovo",
