...
static int kysona_battery_probe(struct hid_device *hdev)
{
	...
	/*
	 * Linux power-supply registration publishes the new class device. The
	 * reduced firmware boundary needs an explicit change to emit its initial
	 * detached snapshot.
	 */
			    ...
			    struct hid_report *report, u8 *data, int size)
{
	...
	if (size == sizeof(kysona_online_request) &&
		...
		/*
		 * Upstream Linux reads this state directly through power-supply
		 * sysfs. The firmware boundary publishes detached snapshots only
		 * after power_supply_changed().
		 */
		if (drv_data->battery)
			power_supply_changed(drv_data->battery);
	}
...
static const struct hid_driver kysona_driver = {
	.name			= "kysona",
