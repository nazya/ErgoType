...
//
// #include <linux/hid.h>
// Pico port uses the local Linux compatibility header path.
#include "../../include/linux/hid.h"

// static struct hid_driver hid_generic;
// Port stores imported HID driver descriptors in flash/rodata.
static const struct hid_driver hid_generic;
...
#if 0
	/*
	 * Temporarily disabled in callback-driven generic HID mode. HIDRAW/FF are
	 * useful for proxy later, but their buffering/control paths need explicit
	 * async ownership before they run from TinyUSB callbacks.
	 */
