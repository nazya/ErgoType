...
// Firmware exposes hid_to_usb_dev() and the task-side USB message bridge
// through the reduced public hid.h/usb.h contract.
#include "hid-ids.h"
...
#define ADD_DESC(_desc_ptr, _desc_size) \
	do {                                                        \
		...
		/* FreeRTOS heap_4 has no size-aware realloc contract. */ \
		new_desc = kmalloc(new_size, GFP_KERNEL);           \
		if (new_desc == NULL) {                             \
			goto cleanup;                               \
		}                                                   \
		if (size != 0)                                      \
			memcpy(new_desc, desc, size);                \
		kfree(desc);                                        \
