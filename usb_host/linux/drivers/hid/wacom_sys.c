...
static int wacom_hid_set_device_mode(struct hid_device *hdev)
{
	...
	// Firmware report enums use a bounded sparse registry instead of a
	// 256-entry hash.
	r = hid_report_enum_lookup(re, hid_data->inputmode);
				 ...
				 struct wacom_wac *wacom_wac)
{
	...
	// Firmware report enums use a bounded sparse registry.
	r = hid_report_enum_lookup(re, wacom_wac->mode_report);
	if (!r)
		return -EINVAL;
...
// Probe/remove and their devres callbacks run only in usbhid_lifecycle_task.
...
static void wacom_release_shared_data(struct kref *kref)
{
	...
	// The firmware lifecycle task serializes this shared-list update.
	list_del(&data->list);
...
static int wacom_add_shared_data(struct hid_device *hdev)
{
	...
	// mutex_lock(&wacom_udev_list_lock);
	// The firmware lifecycle task serializes lookup and publication.
