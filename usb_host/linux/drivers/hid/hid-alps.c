	...
	u8 *read_val, u8 write_val, bool read_flag)
{
	...
	if (read_flag) {
		...
		// RP2040 cannot rely on the unaligned 32-bit load at offset 6.
		if (get_unaligned_le32(readbuf + 6) != address) {
			dev_err(&hdev->dev, "read register address error (%x,%x)\n",
				// *(u32 *)&readbuf[6], address);
				get_unaligned_le32(readbuf + 6), address);
			goto exit_readbuf;
		}
		...
		// Offset 13 is unaligned on RP2040; preserve the same LE value.
		if (get_unaligned_le16(readbuf + 13) != check_sum) {
			dev_err(&hdev->dev, "read register checksum error (%x,%x)\n",
				// *(u16 *)&readbuf[13], check_sum);
				get_unaligned_le16(readbuf + 13), check_sum);
			goto exit_readbuf;
		}
...
static int alps_input_configured(struct hid_device *hdev, struct hid_input *hi)
{
	...
	if (data->has_sp) {
		...
		// The upstream driver has no remove hook for this secondary input;
		// bind its lifetime to the HID device in the reduced device core.
		input2 = devm_input_allocate_device(&hdev->dev);
		if (!input2) {
			ret = -ENOMEM;
			goto exit;
		}
...
// Firmware stores imported HID driver descriptors in flash/rodata.
static const struct hid_driver alps_driver = {
	.name = "hid-alps",
	.id_table		= alps_id,
	.probe			= alps_probe,
