				...
				struct hidpp_report *hidpp_report)
{
	...
	// Preserve firmware's exact disconnect status so the generation-bound
	// waiter can terminate rather than turning unplug into a generic retry.
	if (ret < 0)
		return ret;
	return ret == fields_count ? 0 : -1;
}
	...
	struct hidpp_report *response)
{
	 ...
	 */
	// *response = *message;
	// Firmware must initialize the matching question before publishing its
	// pointer to the independently scheduled report task.
	*response = *message;
	hid_compat_waitqueue_state_lock(&hidpp->wait);
	hidpp->send_receive_buf = response;
	// hidpp->answer_available = false;
	// The report task can run on the other core; publish the durable predicate.
	__atomic_store_n(&hidpp->answer_available, false, __ATOMIC_RELEASE);
	...
	if (ret) {
		...
		// Clear only after the report task can no longer own this buffer.
	...
	// 			5*HZ)) {
	// Firmware includes exact-interface disconnect in the durable condition,
	// then maps it outside the Linux-compatible wait return contract.
	wait_ret = wait_event_timeout(hidpp->wait,
				      hid_compat_waitqueue_cancelled(
					      &hidpp->wait) ||
				      __atomic_load_n(&hidpp->answer_available,
						      __ATOMIC_ACQUIRE),
				      5*HZ);
	if (hid_compat_waitqueue_cancelled(&hidpp->wait)) {
		discard_response = true;
		ret = -ENODEV;
		goto out;
	}
	...
	struct hidpp_report *response)
{
	...
	do {
		...
		/*
		 * Upstream Linux: the response-ID checks below normally select BUSY
		 * retry. Firmware clears a canceled stack response only after fencing
		 * the report task, so preserve exact-interface unplug as terminal.
		 */
		if (ret == -ENODEV)
			break;
		if (response->report_id == REPORT_ID_HIDPP_SHORT &&
		...
		int size)
{
	...
	if (unlikely(mutex_is_locked(&hidpp->send_mutex))) {
		/*
		 * Upstream reads send_receive_buf without a state lock. Firmware's
		 * report task can run on the other core, so the lock protects the
		 * published stack pointer through every matching read or copy below.
		 */
		hid_compat_waitqueue_state_lock(&hidpp->wait);
		...
		// if (!question)
		// 	return 0;
		// Release the firmware state lock before preserving upstream's return.
		if (!question) {
			hid_compat_waitqueue_state_unlock(&hidpp->wait);
			return 0;
		}
		...
		if (hidpp_match_answer(question, report) ||
			...
			*answer = *report;
			...
			// Pair the cross-core report copy with the task's acquire load.
			__atomic_store_n(&hidpp->answer_available, true,
					 __ATOMIC_RELEASE);
			hid_compat_waitqueue_state_unlock(&hidpp->wait);
			wake_up(&hidpp->wait);
			/*
	...
	/*
	 * The first firmware checkpoint covers only direct request/reply. Battery,
	 * receiver connect events, wheel extensions, and vendor-key handling stay
	 * in their upstream positions below for later separately tested stages.
	 */
	return 0;
#else
...
#if IS_ENABLED(CONFIG_HID_LOGITECH_HIDPP_DIRECT_REQUEST_REPLY)
	/*
	 * The reduced production stage retains only upstream protocol detection.
	 * Identity, battery, scrolling, and delayed input remain separately gated.
	 */
	(void)ret;
	return;
#else
...
static int hidpp_get_report_length(struct hid_device *hdev, int id)
{
	...
	struct hid_report *candidate;

	re = &(hdev->report_enum[HID_OUTPUT_REPORT]);
	// report = re->report_id_hash[id];
	// RP2040 keeps the complete report-ID domain in the sparse upstream list.
	report = NULL;
	list_for_each_entry(candidate, &re->report_list, list) {
		if (candidate->id == id) {
			report = candidate;
			break;
		}
...
#if IS_ENABLED(CONFIG_HID_LOGITECH_HIDPP_DIRECT_REQUEST_REPLY)
	/* No battery/sysfs publication exists in the request/reply checkpoint. */
#else
	sysfs_remove_group(&hdev->dev.kobj, &ps_attribute_group);
#endif
...
#if IS_ENABLED(CONFIG_HID_LOGITECH_HIDPP_DIRECT_REQUEST_REPLY)
	/*
	 * First checkpoint: one direct USB identity only. Receiver children,
	 * Bluetooth, wheels, touchpads, and broad Logitech matching remain below
	 * in the upstream table for later independently measured stages.
	 */
...
// Firmware keeps mutable registration state in hid_driver_runtime.
static const struct hid_driver hidpp_driver = {
	.name = "logitech-hidpp-device",
	.id_table = hidpp_devices,
#if IS_ENABLED(CONFIG_HID_LOGITECH_HIDPP_DIRECT_REQUEST_REPLY)
	/*
	 * Input remains generic while the first checkpoint adds only protocol
	 * request/reply. Capability-specific fixup/mapping/event hooks stay in
	 * their upstream descriptor below.
	 */
	.probe = hidpp_probe,
	.remove = hidpp_remove,
