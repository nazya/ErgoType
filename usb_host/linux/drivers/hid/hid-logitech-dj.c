...
// Upstream Linux: no equivalent; virtual children need firmware evdev activation.
#include "usb_host/evdev.h"
// Upstream Linux workqueues do not own the firmware HID device registry.
...
struct dj_receiver_dev {
	...
	// spinlock_t lock;
	// FreeRTOS report and lifecycle tasks use a task mutex for this state.
	struct mutex lock;
};
...
// The firmware lifecycle task serializes receiver registry add/remove.
...
static void dj_release_receiver_dev(struct kref *kref)
{
	...
	// Upstream Linux: the embedded spinlock needs no destruction.
	mutex_destroy(&djrcv_dev->lock);
	kfree(djrcv_dev);
}
...
static void dj_put_receiver_dev(struct hid_device *hdev)
{
	...
	// The firmware lifecycle task owns every call touching dj_hdev_list.
						   ...
						   bool is_hidpp)
{
	...
	if (!djrcv_dev) {
		...
		// FreeRTOS mutex storage is allocated explicitly and may exhaust.
		mutex_init(&djrcv_dev->lock);
		if (!mutex_initialized(&djrcv_dev->lock)) {
			kfree(djrcv_dev);
			djrcv_dev = NULL;
			goto out;
		}
					      ...
					      struct dj_workitem *workitem)
{
	...
	// unsigned long flags;
	// The FreeRTOS task mutex does not use interrupt-state flags.

	// spin_lock_irqsave(&djrcv_dev->lock, flags);
	// The report and lifecycle tasks share paired-device state.
	mutex_lock(&djrcv_dev->lock);
	dj_dev = djrcv_dev->paired_dj_devices[workitem->device_index];
	djrcv_dev->paired_dj_devices[workitem->device_index] = NULL;
	// spin_unlock_irqrestore(&djrcv_dev->lock, flags);
	// End the task-mutex replacement for this upstream critical section.
	mutex_unlock(&djrcv_dev->lock);
...
static void delayedwork_callback(struct work_struct *work)
{
	...
	// spin_lock_irqsave(&djrcv_dev->lock, flags);
	// The report and lifecycle tasks share the notification FIFO.
	mutex_lock(&djrcv_dev->lock);
	...
	if (!kfifo_is_empty(&djrcv_dev->notif_fifo))
		...
		// Virtual HID add/remove must run on the firmware lifecycle owner.
					    ...
					    struct dj_workitem *workitem)
{
	...
	case REPORT_TYPE_KEYBOARD:
		async_msg("INFO: HID_USAGES675_UP_12288");
				    ...
				    struct dj_report *dj_report)
{
	...
	// RP2040 stores report IDs in the sparse report list to save per-device RAM.
	report = hid_report_enum_lookup(output_report_enum, REPORT_ID_DJ_SHORT);
			 ...
			 const struct hid_device_id *id)
{
	...
	// RP2040 stores report IDs in the sparse report list to save per-device RAM.
	rep = hid_report_enum_lookup(output_report_enum, REPORT_ID_DJ_SHORT);
	...
	if (has_hidpp) {
		// spin_lock_irqsave(&djrcv_dev->lock, flags);
		// The report and lifecycle tasks share receiver readiness.
		mutex_lock(&djrcv_dev->lock);
...
static void logi_dj_remove(struct hid_device *hdev)
{
	...
	// Receiver work runs on this firmware lifecycle owner, not system_wq.
	usbhid_lifecycle_cancel_work(&djrcv_dev->work);
...
// The firmware HID registry stores immutable driver definitions.
static const struct hid_driver logi_djreceiver_driver = {
