	...
	const struct hid_device_id *id)
{
	...
	/*
	 * Firmware links only the USB Mouse 2/Trackpad 2 IDs below. Mouse 2
	 * returns immediately after hid_hw_start(), while Trackpad 2 can issue
	 * the mode SET but never enters the Mouse-2-only delayed retry branch.
	 * Keep the unreachable upstream retry visible without enabling a generic
	 * delayed-work lifetime that no linked USB path can arm.
	 */
	...
	if (ret == -EIO && (id->product == USB_DEVICE_ID_APPLE_MAGICMOUSE2 ||
		...
		/* The USB-only match table makes this branch unreachable; see probe. */
	}
...
static void magicmouse_remove(struct hid_device *hdev)
{
	...
	if (msc) {
		...
		/* No linked USB path can initialize or arm the upstream retry work. */
...
static const struct hid_device_id magic_mice[] = {
	...
	/* Firmware has no Bluetooth HID transport; retain only USB work devices. */
...
/* The firmware has no module unload or runtime driver mutation; keep in flash. */
static const struct hid_driver magicmouse_driver = {
	.name = "magicmouse",
	.id_table = magic_mice,
