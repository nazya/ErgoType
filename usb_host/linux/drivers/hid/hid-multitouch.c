...
struct mt_application {
	...
	// The port exposes jiffies as a FreeRTOS tick macro, so the member needs a distinct token.
	unsigned long frame_jiffies;	/* the frame's jiffies */
...
static int mt_compute_timestamp(struct mt_application *app, __s32 value)
{
	...
	// The port member is renamed because jiffies is a FreeRTOS tick macro.
	unsigned long jdelta = jiffies_to_usecs(jiffies - app->frame_jiffies);

	// app->jiffies = jiffies;
	// The port member is renamed because jiffies is a FreeRTOS tick macro.
	app->frame_jiffies = jiffies;
