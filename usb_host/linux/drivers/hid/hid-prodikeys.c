...
// Firmware exposes keyboard input only; MIDI/ALSA is outside the selected scope.
#include "hid-ids.h"
...
#if 0
// Upstream MIDI sustain state is retained but not allocated by the keyboard port.
...
#if 0
	// Upstream MIDI state has no firmware consumer.
...
#if 0
	// Upstream ALSA publication is outside the keyboard input boundary.
...
#if 0
// Upstream ALSA registration, MIDI sysfs controls and sustain timers are retained.
...
#if 0
// MIDI-only interpretation of Mail/WWW and musical notes is not enabled.
...
static int pcmidi_handle_report4(struct pcmidi_snd *pm, u8 *data)
{
	...
	for (bit_index = 0; bit_index < 24; bit_index++) {
		...
		case 0x020000: /* midi launcher..send a key (qwerty) or not? */
			...
			// pm->midi_mode ^= 0x01;
			// dbg_hid("pcmidi mode: %d\n", pm->midi_mode);
			// The MIDI launcher does not change keyboard mode in this port.
			continue;
		case 0x100000: /* KEY_MESSENGER or octave up */
#if 0
			// Upstream octave switching belongs to the excluded MIDI mode.
			dbg_hid("pcmidi mode: %d\n", pm->midi_mode);
			if (pm->midi_mode) {
				pm->midi_octave++;
				if (pm->midi_octave > 2)
	...
	struct pcmidi_snd *pm, unsigned report_id, u8 *data, int size)
{
	...
	case 0x01: /* midi keys (qwerty)*/
		...
		// No MIDI mode: let the ordinary Mail/WWW HID report be parsed.
		break;
	case 0x03: /* midi keyboard (musical)*/
		// ret = pcmidi_handle_report3(pm, data, size);
		// Musical notes have no ALSA consumer in the keyboard-only port.
		break;
...
#if 0
// Upstream ALSA lifecycle is retained; probe enables keyboard report 6 directly.
...
static int pk_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	// The keyboard still needs report 6 setup; ALSA/MIDI initialization is omitted.
	ret = pcmidi_set_operational(pm);
	if (ret < 0)
		goto err_stop;
...
static void pk_remove(struct hid_device *hdev)
{
	...
	// No ALSA card or MIDI timers were created by the keyboard-only probe.
	hid_hw_stop(hdev);
...
static const struct hid_driver pk_driver = {
	.name = "prodikeys",
	.id_table = pk_devices,
