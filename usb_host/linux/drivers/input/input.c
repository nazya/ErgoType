 ...
 * support, and more lifecycle code. This port currently keeps the in-memory
 * handler/event path needed by hid-input and the firmware KeyD boundary.
 */
...
struct input_dev *input_find_device_by_name(const char *name)
{
	...
	// Work3 wrapped this lookup in hid_compat_exec_sync(). This slice has no
	// executor task, so list access stays synchronous in the caller context.
	list_for_each_entry(dev, &input_dev_list, node)
...
int input_for_each_device(int (*fn)(struct input_dev *dev, void *data), void *data)
{
	...
	// Work3 wrapped this iteration in hid_compat_exec_sync(). This slice has no
	// executor task, so list access stays synchronous in the caller context.
	list_for_each_entry(dev, &input_dev_list, node) {
		int ret = fn(dev, data);
