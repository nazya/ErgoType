...
commit `00c751e`. Neither path is in the active CMake allowlist.
...
- `device/google-stadiaff`: Google Stadia VID/PID and a report descriptor with
  ...
  short layout-rumble trigger preserved in commit `00c751e`.
...
| Historical force feedback, inactive | `hid-google-stadiaff.c`, `ff-core.c`, and `ff-memless.c` at `00c751e` | `google-stadiaff` |
| Historical timer/HIDDEV-force path, inactive | `hid-appleir.c` at `00c751e` | `apple-ir` |
...
`00c751e`. The active build has no FF driver; standard haptic touchpad coverage
...
- `device/google-stadiaff` (historical, commit `00c751e`)
...
- [ ] `device/holtek-kbd-a055` LED output path
  ...
  - emulator:
  - verdict:
- [x] `device/google-stadiaff` historical FF output path at `00c751e`
  ...
  - verdict: pass, checked on Pico host with emulator.
- [x] `device/apple-ir` historical AppleIR path at `00c751e`
