...
SPI pin validation is target-specific. CS is not validated as an SPI signal; it
is handled as a plain GPIO. The RP2040 bus-pin mappings are:
- `spi0.sck`: `2`, `6`, `18`, `22`
- `spi0.mosi`: `3`, `7`, `19`, `23`
- `spi0.miso`: `0`, `4`, `16`, `20`
- `spi1.sck`: `10`, `14`, `26`
...
RP2350 accepts the corresponding mappings available on the selected chip. ESP
targets use the GPIO matrix and the selected board's GPIO whitelist.
...
- SPI clock: the bus speed is set once via `platform_spi_init()` inside the pointing task.
- IRQ polarity: `irq`/MOT is configured as `PLATFORM_GPIO_IN` with internal pull-up; interrupts are enabled on the falling edge (idle HIGH, pulse/level LOW).
