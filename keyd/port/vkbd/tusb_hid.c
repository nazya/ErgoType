/*
 * ErgoType - HID backend for the keyd vkbd event stream.
 *
 * This nRF build sends reports through platform/hid.
...
{
	platform_hid_nkro_report(nkro_keys, sizeof(nkro_keys));
}
...
static void send_consumer_report(void)
{
	...
	platform_hid_consumer_report(usages, 6);
}

static void mouse_scroll(int16_t x, int16_t y)
{
	platform_hid_mouse_report(mouse_buttons, 0, 0, y, x);
}

static void mouse_move(int16_t x, int16_t y)
{
	platform_hid_mouse_report(mouse_buttons, x, y, 0, 0);
}
...
static void mouse_button(uint8_t buttons, int state)
{
	...
	platform_hid_mouse_report(mouse_buttons, 0, 0, 0, 0);
}
...
void vkbd_hid_nkro_task(void *pvParameters)
{
	...
	while (1) {
		if (platform_vkbd_event_recv(&event, PLATFORM_VKBD_EVENT_WAIT_FOREVER) != 0)
			continue;
