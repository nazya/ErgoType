 ...
 *
 * This file intentionally contains all HID report packing.
...
static void send_boot_keyboard_report(void)
{
	...
	while (!platform_hid_ready(HID_KEYBOARD_INSTANCE))
		vTaskDelay(HID_READY_WAIT_TICKS);

	platform_hid_keyboard_report(HID_KEYBOARD_INSTANCE, 0, mods, usages);
}
...
static void send_nkro_report(void)
{
	...
	platform_hid_report(HID_KEYBOARD_INSTANCE, REPORT_ID_KEYBOARD, nkro_keys, sizeof(nkro_keys));
}
...
static void send_consumer_report(void)
{
	...
	platform_hid_report(HID_KEYBOARD_INSTANCE, REPORT_ID_CONSUMER, usages, sizeof(usages));
}
...
static void mouse_scroll(int16_t x, int16_t y)
{
	...
	while (!platform_hid_ready(HID_NKRO_MOUSE_INSTANCE))
		vTaskDelay(HID_READY_WAIT_TICKS);

	/* TinyUSB: wheel=vertical, pan=horizontal. */
	platform_hid_report(HID_NKRO_MOUSE_INSTANCE, HID_NKRO_MOUSE_REPORT_ID, &report, sizeof(report));
}
...
{
	while (!platform_hid_ready(HID_MOUSE_INSTANCE))
		vTaskDelay(HID_READY_WAIT_TICKS);

	platform_hid_mouse_report(
		HID_MOUSE_INSTANCE,
		0,
		mouse_buttons,
		0,
...
static void mouse_button_boot(uint8_t buttons, int state)
{
	...
	platform_hid_mouse_report(HID_MOUSE_INSTANCE, 0, mouse_buttons, 0, 0, 0, 0);
}
