...
static void free_config(struct config *config)
{
	...
	// vPortFree(config); // platform_large_alloc may use external RAM on ESP.
	platform_large_free(config);
}
...
{
	// struct config *config = pvPortMalloc(sizeof *config); // use the platform policy for this large object.
	struct config *config = platform_large_alloc(sizeof *config);

	if (!config) {
		dbg2("size of struct conf: %u bytes", sizeof(struct config));
		err("kbd was not initialize: platform_large_alloc failed for struct config");
		return;
	}
