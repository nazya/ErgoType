...
int config_parse_file(struct config *config, const char *filename)
{
    ...
    if (platform_storage_size(filename, &file_size) != 0) {
    ...
    if (!buffer) {
        err("failed to allocate %zu bytes for %s", file_size + 1, filename);
        return -1;
    }

    if (platform_storage_read(filename, (uint8_t *)buffer, file_size, &bytes_read) != 0) {
    ...
    for (const char *cursor = buffer; *cursor;) {
        size_t len = 0;
        while (cursor[len] && cursor[len] != '\n')
            len++;
        if (len > MAX_LINE_LEN) {
            err("maximum line length exceeded (%d)", MAX_LINE_LEN);
            free_section_entries(&section);
            vPortFree(buffer);
            return -1;
        }
