...
static inline platform_spi_t *pmw3389_spi(const pmw33xx_cfg_t *cfg)
{
...
{
    platform_spi_t *spi = pmw3389_spi(cfg);
...
static uint8_t read_register(const pmw33xx_cfg_t *cfg, uint8_t reg_addr)
{
    ...
    platform_busy_wait_us_32(PMW3389_TSRAD_US);
    ...
    cs_deselect(cfg);
    platform_busy_wait_us_32(PMW3389_TSRW_US - 1u);
...
static void write_register(const pmw33xx_cfg_t *cfg, uint8_t reg_addr, uint8_t data)
{
    ...
    platform_busy_wait_us_32(PMW3389_TSCLK_NCS_WRITE_US);
    cs_deselect(cfg);
    platform_busy_wait_us_32(PMW3389_TSWW_TSWR_US);
}
...
static void upload_firmware(const pmw33xx_cfg_t *cfg)
{
    ...
    for (int i = 0; i < (int)pmw3389_firmware_length; i++) {
        platform_spi_write_blocking(spi, &(pmw3389_firmware_data[i]), 1);
        platform_busy_wait_us_32(15);
    }
...
void pmw3389_get_deltas(const pmw33xx_cfg_t *cfg, int16_t *dx, int16_t *dy)
{
    ...
    platform_busy_wait_us_32(PMW3389_TSRAD_MOTBR_US);
