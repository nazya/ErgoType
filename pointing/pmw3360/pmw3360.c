...
static inline platform_spi_t *pmw3360_spi(const pmw33xx_cfg_t *cfg)
{
...
{
    platform_spi_t *spi = pmw3360_spi(cfg);
    // PMW3360 uses SPI mode 3 (CPOL=1, CPHA=1).
    platform_spi_set_format(
        spi, 8, PLATFORM_SPI_CPOL_1, PLATFORM_SPI_CPHA_1, PLATFORM_SPI_MSB_FIRST);
}

static void cs_select(const pmw33xx_cfg_t *cfg) {
    asm volatile ("nop \n nop \n nop");
    platform_gpio_put((uint)cfg->cs, 0);        // Active low
    asm volatile ("nop \n nop \n nop");
}

static void cs_deselect(const pmw33xx_cfg_t *cfg) {
    asm volatile ("nop \n nop \n nop");
    platform_gpio_put((uint)cfg->cs, 1);
    asm volatile ("nop \n nop \n nop");
}
...
static uint8_t read_register(const pmw33xx_cfg_t *cfg, uint8_t reg_addr) {
    ...
    platform_spi_write_blocking(spi, &x, 1);
    platform_busy_wait_us_32(100);              // tSRAD
    // read data
    uint8_t data;
    platform_spi_read_blocking(spi, 0, &data, 1);

    platform_busy_wait_us_32(1);                // tSCLK-NCS for read operation is 120ns
    cs_deselect(cfg);
    platform_busy_wait_us_32(19);               // tSRW/tSRR (=20us) minus tSCLK-NCS
...
static void write_register(const pmw33xx_cfg_t *cfg, uint8_t reg_addr, uint8_t data) {
    ...
    platform_spi_write_blocking(spi, &x, 1);
    // send data
    platform_spi_write_blocking(spi, &data, 1);

    platform_busy_wait_us_32(20);               // tSCLK-NCS for write operation
    cs_deselect(cfg);
    platform_busy_wait_us_32(100);              // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound 
}
...
static void upload_firmware(const pmw33xx_cfg_t *cfg) {
    ...
    platform_spi_write_blocking(spi, &data, 1);
    platform_busy_wait_us_32(15);

    // send all bytes of the firmware
    for (int i = 0; i < firmware_length; i++) {
        platform_spi_write_blocking(spi, &(firmware_data[i]), 1);
        platform_busy_wait_us_32(15);
    }
...
void pmw3360_get_deltas(const pmw33xx_cfg_t *cfg, int16_t *dx, int16_t *dy) {
    ...
    platform_spi_write_blocking(spi, &address, 1);
    platform_busy_wait_us_32(35);

    uint8_t data[Motion_Burst_Size];
    platform_spi_read_blocking(spi, 0, data, sizeof(data));

    platform_busy_wait_us_32(1);
    cs_deselect(cfg);
    platform_busy_wait_us_32(1);
...
bool pmw3360_init(const pmw33xx_cfg_t *cfg) {
    platform_gpio_init((uint)cfg->cs);
    platform_gpio_set_dir((uint)cfg->cs, PLATFORM_GPIO_OUT);
    platform_gpio_put((uint)cfg->cs, 1);
