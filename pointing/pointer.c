...
static platform_spi_t *const spi_by_idx[MAX_SPI] = { &platform_spi0, &platform_spi1 };
...
static motion_irq_pin_t mot_irq_pins[MAX_PMW3360 + MAX_PMW3389];
static uint8_t mot_irq_pin_count;

static uint32_t mot_gpio_bits(uint gpio)
{
    for (uint8_t i = 0; i < mot_irq_pin_count; ++i) {
        if (mot_irq_pins[i].gpio == gpio)
            return mot_irq_pins[i].bits;
    }
    return 0;
}

static bool mot_gpio_add_sensor(int8_t mot_pin, uint8_t sensor_idx)
{
    if (mot_pin < 0)
        return false;

    const uint gpio = (uint)mot_pin;
    const uint32_t bit = 1u << sensor_idx;

    for (uint8_t i = 0; i < mot_irq_pin_count; ++i) {
        if (mot_irq_pins[i].gpio == gpio) {
            mot_irq_pins[i].bits |= bit;
            return true;
        }
    }

    if (mot_irq_pin_count >= (MAX_PMW3360 + MAX_PMW3389)) {
        err("pointing: too many motion IRQ pins");
        return false;
    }

    mot_irq_pins[mot_irq_pin_count++] = (motion_irq_pin_t) {
        .gpio = gpio,
        .bits = bit,
    };
    return true;
...
{
    platform_gpio_init((uint)mot_pin);
    platform_gpio_set_dir((uint)mot_pin, PLATFORM_GPIO_IN);
    platform_gpio_pull_up((uint)mot_pin); // idle HIGH

    if (!mot_irq_callback_installed) {
        platform_gpio_set_irq_enabled_with_callback((uint)mot_pin, PLATFORM_GPIO_IRQ_EDGE_FALL, true, mot_irq_handler);
        mot_irq_callback_installed = true;
    } else {
        platform_gpio_set_irq_enabled((uint)mot_pin, PLATFORM_GPIO_IRQ_EDGE_FALL, true);
    }
}
...
void pointing_motion_irq_init(TaskHandle_t task_handle, int8_t mot_pin, uint8_t sensor_idx)
{
    ...
    if (!mot_gpio_add_sensor(mot_pin, sensor_idx))
        return;
...
{
    platform_spi_t *spi = spi_by_idx[bus];
    int8_t sck = config->spi[bus].sck;
    int8_t mosi = config->spi[bus].mosi;
    int8_t miso = config->spi[bus].miso;

    platform_gpio_set_function((uint)sck, PLATFORM_GPIO_FUNC_SPI);
    platform_gpio_set_function((uint)mosi, PLATFORM_GPIO_FUNC_SPI);
    platform_gpio_set_function((uint)miso, PLATFORM_GPIO_FUNC_SPI);

    platform_spi_init(spi, (uint)config->spi[bus].baud);
}
