...
#define GPIO_PULL_UP(pin)       \
    do {                        \
        platform_gpio_input_pullup(pin); \
    } while (0)

#define GPIO_PULL_DOWN(pin)     \
    do {                        \
        platform_gpio_input_pulldown(pin); \
    } while (0)
...
{
    if (!platform_gpio_is_valid((platform_gpio_t)pin)) {
...
static bool spi_pin_valid(uint8_t spi_idx, const char *name, int pin)
{
    (void)spi_idx;
    (void)name;
    return platform_gpio_is_valid((platform_gpio_t)pin);
}
...
#ifdef ERGOTYPE_ZEPHYR
    size_t file_size;
    int rc = platform_storage_size(filename, &file_size);
    if (rc || file_size == 0) {
        err("Failed to get file size for %s (rc=%d)", filename, rc);
        return -1;
    }

    *buffer = (char *)pvPortMalloc(file_size + 1);
    if (!*buffer) {
    ...
    size_t bytes_read = 0;
    rc = platform_storage_read(filename, (uint8_t *)*buffer, file_size, &bytes_read);
    if (rc || bytes_read != file_size) {
        err("Failed to read %s (rc=%d)", filename, rc);
        vPortFree(*buffer);
        *buffer = NULL;
        return -1;
    }
