...
#include "platform/storage.h" // FatFS header
#include "core_json.h"
#include "jconfig.h"
#include "keys.h"
...
static uint64_t used_gpio_mask;
...
static bool claim_gpio(int pin)
{
    ...
    uint64_t bit = 1ULL << (uint32_t)pin;
...
static bool spi_pin_valid(uint8_t spi_idx, const char *name, int pin)
{
    if (strcmp(name, "sck") == 0)
        return platform_gpio_is_valid_for_spi(spi_idx, PLATFORM_GPIO_SPI_SCK, pin);
    if (strcmp(name, "mosi") == 0)
        return platform_gpio_is_valid_for_spi(spi_idx, PLATFORM_GPIO_SPI_MOSI, pin);
    if (strcmp(name, "miso") == 0)
        return platform_gpio_is_valid_for_spi(spi_idx, PLATFORM_GPIO_SPI_MISO, pin);
    return false;
}
...
{
    if (strcmp(name, "tx") == 0)
        return platform_gpio_is_valid_for_uart(uart_idx, PLATFORM_GPIO_UART_TX, pin);
    if (strcmp(name, "rx") == 0)
        return platform_gpio_is_valid_for_uart(uart_idx, PLATFORM_GPIO_UART_RX, pin);
    return false;
}
...
static bool i2c_pin_valid(uint8_t i2c_idx, const char *name, int pin)
{
    if (strcmp(name, "sda") == 0)
        return platform_gpio_is_valid_for_i2c(i2c_idx, PLATFORM_GPIO_I2C_SDA, pin);
    if (strcmp(name, "scl") == 0)
        return platform_gpio_is_valid_for_i2c(i2c_idx, PLATFORM_GPIO_I2C_SCL, pin);
    return false;
}
...
static int load(char **buffer, const char *filename) {
    ...
    platform_fs_result_t res;
    uint32_t bytesRead;
    size_t file_size;

    // Mount the filesystem
    (void)platform_storage_mount();
    res = platform_fs_mount(&filesystem, PLATFORM_FS_MOUNT_NOW);
    ...
    res = platform_file_open(&file, filename, PLATFORM_FS_READ);
    if (res != PLATFORM_FS_OK) {
    ...
    res = platform_file_stat(filename, &fno);
    if (res != PLATFORM_FS_OK) {
    ...
    *buffer = (char *)pvPortMalloc(file_size + 1);  // +1 for null-terminator
    ...
    res = platform_file_read(&file, *buffer, (uint32_t)file_size, &bytesRead);
    if (res != PLATFORM_FS_OK || bytesRead != file_size) {
    ...
    platform_file_close(&file);  // Close the file
    platform_fs_unmount();  // Unmount the filesystem
